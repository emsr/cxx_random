...
template<std::size_t Dim>
  ...
  insphplot(std::ostream & iv, double radius = 1.0)
  {
    ...
    __gnu_test::uniform_inside_sphere_distribution<Dim> isd(radius);
...
main()
{
  __gnu_test::uniform_inside_sphere_distribution<3> insph;
  std::cout << "default insph = " << insph << '\n';

  __gnu_test::uniform_inside_sphere_distribution<2> incirc;
